    ...
    <x-slot name="header">
        <div class="flex items-center gap-6 px-4">
            ...
            <div>
                ...
                <p class="text-[8px] text-blue-500 font-black uppercase tracking-[0.4em] mt-1 italic">Colección Premium / ID: {{ $producto->id }}</p>
            </div>
        </div>
    </x-slot>
                ...
                <form action="{{ route('producto.update', $producto->id) }}" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="grid grid-cols-1 lg:grid-cols-2 gap-12">
                        ...
                        <div class="space-y-8">
                            ...
                            <div>
                                ...
                                <select name="categoria_id" id="categoria_id" 
                                    ...
                                    @foreach($categorias as $categoria)
                                        <option value="{{ $categoria->id }}" {{ $producto->categoria_id == $categoria->id ? 'selected' : '' }} class="bg-[#111] text-white">
                                            {{ $categoria->nombre }}
                                        </option>
                                    @endforeach
                                </select>
                            ...
                            <div class="grid grid-cols-2 gap-6">
                                <div>
                                    ...
                                    <input id="precio" name="precio" type="number" step="0.01" value="{{ $producto->precio }}"
                                ...
                                <div>
                                    ...
                                    <input id="stock" name="stock" type="number" value="{{ $producto->stock }}"
                                        class="block w-full bg-white/5 border border-white/10 rounded-2xl text-white px-6 py-4 text-lg font-black focus:ring-1 focus:ring-blue-500 focus:border-blue-500 transition-all italic"
                                        required />
                                </div>
                            </div>
                        ...
                        <div class="flex flex-col h-full">
                            ...
                            <div x-data="{ photoPreview: null, currentPhoto: '{{ $producto->imagen_url ? asset('storage/' . $producto->imagen_url) : '' }}' }" class="relative flex-1 group min-h-[300px]">
                                ...
                                <div class="w-full h-full bg-white/[0.03] rounded-[2.5rem] border-2 border-dashed border-white/20 flex flex-col items-center justify-center cursor-pointer hover:bg-white/[0.07] transition-all duration-500 relative overflow-hidden" 
                                    ...
                                    <div x-show="!photoPreview && !currentPhoto" class="text-center p-8">
                                        ...
                                        <p class="text-[8px] text-gray-500 font-black uppercase tracking-widest italic">Cargar Foto</p>
                                    </div>
                                    ...
                                    <div x-show="!photoPreview && currentPhoto" class="absolute inset-0">
                                        <img x-bind:src="currentPhoto" class="w-full h-full object-cover">
                                        <div class="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center">
                                            <span class="px-4 py-2 bg-white text-black text-[8px] font-black uppercase tracking-widest rounded-full shadow-2xl">Reemplazar Visual</span>
                                        </div>
                                    </div>
                                </div>
                            </div>
